...
export class ContrastiveLearner {
  ...
  computeLocalContrastive(
    nodeEmbeddings: Float32Array[],
    edges: Edge[],
  ): number;
  addSpectralRegularization(
...
export class TemporalHyperedgeStore {
  ...
  queryByTimeRange(
    ...
    granularity?: TemporalGranularity,
  ): Promise<TemporalHyperedge[]>;
  getActiveHyperedges(at: Date): Promise<TemporalHyperedge[]>;
  aggregateByGranularity(
    hyperedges: TemporalHyperedge[],
    target: TemporalGranularity,
  ): TemporalHyperedge[];
}
```
