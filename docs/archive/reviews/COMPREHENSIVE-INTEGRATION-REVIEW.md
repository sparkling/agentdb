...
const queryText = fromEpisode
  ? `${fromEpisode.task}: ${fromEpisode.output}`
  : "";
...
if (queries.length / dim !== Math.floor(queries.length / dim)) {
  throw new Error(
    `Invalid query dimensions: ${queries.length} not divisible by ${dim}`,
  );
}
```
