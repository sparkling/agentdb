...
const stmt = db.prepare(
  "SELECT * FROM episodes WHERE session_id = ? AND reward > ?",
);
const results = stmt.all(sessionId, minReward);

// ❌ WRONG: String concatenation
const stmt = db.prepare(
  `SELECT * FROM episodes WHERE session_id = '${sessionId}'`,
);

// ❌ WRONG: Template literals with user input
const stmt = db.prepare(
  `SELECT * FROM episodes WHERE session_id = '${userInput}'`,
);
```
...
const stmt = db.prepare(
  `SELECT ${validatedColumn} FROM ${validatedTable} WHERE id = ?`,
);
...
const stmt = db.prepare(
  `SELECT * FROM episodes WHERE id IN (${ids.join(",")})`,
);
```
...
const type = validateEnum(userInput.type, "type", [
  "episode",
  "skill",
  "pattern",
] as const);
...
export function searchEpisodes(
  ...
  try {
    ...
    const validatedSuccess = validateBoolean(
      onlySuccessful,
      "onlySuccessful",
      false,
    );
