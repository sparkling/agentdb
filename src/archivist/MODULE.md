...
> **Footnote (ADR-0253 amendment, 2026-05-25; supersedes ADR-0246 F-03-002 partial-discharge framing)**: mutation-invariants now have three substrate-specific shapes per `[[ADR-0253]]` §"Per-carve-out disposition". (1) **FS-JSON** (`staging-substrate.ts`, post-`cc879ce`): the dispatch path's FS-JSON entries commit *inside* the substrate's `withWrite` file lock — invariants run on already-committed bytes, not on staged state. This is the C3 carve-out in `[[ADR-0253]]`: permanent by design, because `[[ADR-0123]]`'s 100% durability bar takes precedence over deferred-invariants for whole-document FS-JSON. The pre-`cc879ce` staged-then-commit shape that ADR-0246 F-03-002 path (a) had restored for FS-JSON was carved back out by commits `cc879ce` + `bda4669` (2026-05-25); the invariants-on-staged-state property is **LOST for FS-JSON only**. (2) **RVF**: enforcement is **pending** `freeze()` + rollback wiring into archivist dispatch (`RvfBackend.ts:freeze()` exists at `:594` but `grep "freeze" forks/agentdb/src/archivist/` returns zero hits today). RVF-routed invariants run on `undefined` substrate args today; well-formed-payload invariants still fire (pre-fork zero-input guard, dimension checks, etc.). (3) **SQLite carve-outs** (`[[ADR-0166]]`): retain full staged-state invariants. Per `[[feedback-best-effort-must-rethrow-fatals]]`, all three shapes are honest-gap, not silent-drift; the trade-offs are documented inline in `staging-substrate.ts:17-32, 96-97, 367-385` and in `[[ADR-0253]]` §Evidence.
