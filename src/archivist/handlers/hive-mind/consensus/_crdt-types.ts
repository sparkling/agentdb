// charter: dispatch
// ADR-0184 Wave 5 — vendored verbatim from
// `forks/ruflo/v3/@claude-flow/cli/src/mcp-tools/crdt-types.ts` per ADR-0184
// Open Follow-up #2 resolution (option (a): vendor into agentdb, not
// option (b): capability surface). Rationale: pure JSON-merge math, no I/O
// dependencies, no second consumer, capability-handle plumbing is overhead.
// Cli `crdt-types.ts` stays load-bearing through Wave 6 — same vendor pattern
// as Wave 2's `_shared.ts`. Wave 6's scope is cli `hive-mind_consensus`
// handler retirement; cli's `crdt-types.ts` deletion is a separate cli
// housekeeping concern.
//
// Underscore prefix matches `_shared.ts` "internal to consensus" convention.
