...
export class StreamingEmbeddingService extends EnhancedEmbeddingService {
  ...
  constructor(config: Partial<StreamingConfig> = {}) {
    ...
    const resolvedModel = config.model ?? embCfg.model ?? 'Xenova/all-mpnet-base-v2';
    super({ model: resolvedModel, dimension: embCfg.dimension, provider: embCfg.provider, ...config } as StreamingConfig);
