/**
 * ADR-0177 Phase 1.6 (c)/(d)/(e): canonical config-chain accessor.
 *
 * Reads `.claude-flow/embeddings.json` (walking up from cwd) and surfaces
 * embedding model/dimension/provider plus the paid-provider gate. Mirrors the
 * shape of `@claude-flow/memory/resolve-config.ts` `getConfig()`, but stays
 * inside agentdb to avoid a circular dependency (memory already depends on
 * agentdb — agentdb cannot depend on memory).
 *
 * Boot-time consumers (AgentDB.initialize, RvfBackend, EmbeddingService) call
...
/**
 * Convenience exported as `getEmbeddingConfig` so `@claude-flow/memory`
 * `resolve-config.ts` can call `require('agentdb').getEmbeddingConfig()` and
 * layer its own overrides on top. Returns plain (non-frozen) shape per the
 * memory-side caller's expectations.
 */
