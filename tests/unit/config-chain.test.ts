/**
 * ADR-0177 Phase 1.6 (c)/(d)/(e): config-chain accessor + typed errors.
 *
 ...
 *
 * Per Amendment 2 (2026-05-12): does NOT exercise @xenova availability checks.
 */
...
} from '../../src/core/config-chain.js';

let prevCwd: string;
let scratchDir: string;

function makeScratch(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'agentdb-cfgchain-'));
}
...
describe('config-chain: validateBoot', () => {
  ...
  it('throws ConfigChainValidationError when embeddings.json exists but model is missing', () => {
    // explicitly write a file without the model field
    writeEmbeddingsJson(scratchDir, {
      dimension: 768,
      provider: 'onnx',
    });
    // overwrite to remove the default fallback path — file present, model absent
    fs.writeFileSync(
